...
game::game() : current_player{colour::white}, next_player{colour::black}, chess_board(), game_is_active{true},
...
void game::handle_move() {
    ...
    while (asking_for_valid_move) {
        try {
            ...
            if (piece_belongs_to_player&&(valid_king_side_castle_possible||valid_queen_side_castle_possible)&&chess_board.find_king_position(current_player)==initial_position) {
...
std::pair<bool,bool> game::can_validly_castle(const position& king_position) const {
    ...
    const bool in_position{(king_position==position(std::pair<int,int>{0,4})||king_position==position(std::pair<int,int>{7,4}))};
    ...
    if (has_castling_rights && in_position) {
        ...
        while (searching) {
            ...
            if (chess_board.position_in_range(visiting)) {
                ...
                } else if (chess_board.get_piece(visiting)->get_icon()==castle_icon&&visiting.get_position().second==7) {
        ...
        while (searching) {
            ...
            if (chess_board.position_in_range(visiting)) {
                ...
                } else if (chess_board.get_piece(visiting)->get_icon()==castle_icon&&visiting.get_position().second==0) {
                    // If not empty, it could be our rook. If so, our path is clear!
    ...
    if (can_promote_pawn) {
        ...
        while (asking_for_piece) {
            try {
                ...
                std::cout<<"Input the piece you'd like to promote your pawn to; 'q' for Queen; 'r' for Rook; 'k' for Knight; and 'b' for Bishop: ";
                std::cin>>promotion;
