...
auto operator<<(std::ostream& output_stream, const Board& board) noexcept -> std::ostream&
{
    ...
    for (auto rank{board.ranks}; rank >= 1; --rank) {
        ...
        for (auto file{1}; file <= board.files; ++file) {
            output_stream << " ";
            // Check if there is a piece at this position, and if so, print it; otherwise, print a dot
            for (const auto& piece : board.pieces) {
                // There should only be one piece at each position, so we break after printing it
                if (piece.get_position() == std::pair<int, int>{rank, file}) {
                    output_stream << piece;
                    break;
                }
                if (&piece == &board.pieces.back()) {
                    output_stream << "·";
                }
            }
        }
